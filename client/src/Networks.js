...
class Networks extends Component {
  ...
  async componentDidMount() {
    ...
    fetch(api,{method: 'post',
      ...
      .then(res => res.json())
      .then(networks => {
        this.setState({clusterNetworks:networks});
       });
  }
                              ...
                              <div className="row">
                                {this.state.clusterNetworks.map(networkName =>
                                  <div className="font-icon-list col-lg-2 col-md-3 col-sm-4 col-xs-6 col-xs-6">
                                      <a href={"/network-panel/" + networkName}><div className="font-icon-detail"><i className="pe-7s-global"></i>
                                      <input type="text" value={networkName}></input>
                                    </div>  </a>
                                  </div>
                                 )}
                                </div>
